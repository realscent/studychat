...
채팅창의 `첨부` 버튼으로 파일을 보낼 수 있습니다. 여러 파일을 한 번에 선택하면 최대 4개씩 묶음 메시지로 표시됩니다. 이미지는 썸네일을 클릭하면 큰 화면 모달로 열리고, 동영상은 모달에서 플레이어로 재생됩니다. 일반 파일은 다운로드 링크로 표시됩니다. 화면 캡처나 복사한 이미지는 채팅 입력창에 붙여넣으면 바로 전송됩니다. 기본 업로드 제한은 파일당 50MB이며 `MAX_UPLOAD_BYTES` 환경 변수로 조정할 수 있습니다.
